...
router.post('/login', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const token = jwt.sign(
      { id: user.id, email: user.email, role: user.role, name: `${user.firstName || ""} ${user.lastName || ""}`.trim() },
      process.env.NEXTAUTH_SECRET || "fallback_development_secret_do_not_use_in_prod",
      { expiresIn: '30d' }
    );
