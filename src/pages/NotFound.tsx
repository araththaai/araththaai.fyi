    ...
    <div className="py-24 bg-surface min-h-[85vh] flex items-center justify-center">
      <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <h2 className="text-3xl font-heading font-bold text-primary mb-4">
          {language === "en" ? "Jurisdiction Not Found" : "பக்கம் கண்டறியப்படவில்லை"}
        </h2>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/">
            <Button size="lg" className="w-full sm:w-auto bg-primary text-primary-foreground hover:bg-primary/90 h-12 px-6 cursor-pointer">
              <Home className="mr-2 h-4 w-4" /> {language === "en" ? "Return Home" : "முகப்பிற்குச் செல்ல"}
            </Button>
          </Link>
          <Link to="/practice-areas">
            <Button size="lg" variant="outline" className="w-full sm:w-auto border-border text-foreground hover:bg-muted h-12 px-6 cursor-pointer">
              {language === "en" ? "View Practice Areas" : "சட்டப் பிரிவுகளைப் பார்க்க"} <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </Link>
        </div>
      </div>
