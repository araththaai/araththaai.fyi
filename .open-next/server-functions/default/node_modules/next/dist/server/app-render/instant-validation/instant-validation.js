...
_export(exports, {
    ValidationPrefetchKind: function() {
...
const _instantconfig = require("./instant-config");
...
const SEGMENT_STAGE_ORDER = [
    ...
    _stagedrendering.RenderStage.ShellRuntime,
    _stagedrendering.RenderStage.Runtime,
    _stagedrendering.RenderStage.Dynamic
];
async function collectStagedSegmentData(ComponentMod, renderFlightStream, fullPageChunks, fullPageDebugChunks, startTime, clientReferenceManifest, createDebugChannel) {
    ...
    /** Track when we advance stages so we can pass them as `endTime` later. */ const stageEndTimes = {
        [_stagedrendering.RenderStage.Static]: Infinity,
        [_stagedrendering.RenderStage.ShellRuntime]: Infinity,
        [_stagedrendering.RenderStage.Runtime]: Infinity
    };
    ...
    const advanceStage = (targetStage)=>{
        const { currentStage } = controller;
        if (currentStage !== _stagedrendering.RenderStage.Dynamic) {
    ...
    }, ()=>advanceStage(_stagedrendering.RenderStage.ShellRuntime), ()=>advanceStage(_stagedrendering.RenderStage.Runtime), ()=>advanceStage(_stagedrendering.RenderStage.Dynamic));
 ...
 * */ function createStagedStreamFromChunks(stageChunks) {
    ...
    const stream = new _nodestream.Readable({
        read () {
            // Emit static chunks
            emitNewChunks(stageChunks[_stagedrendering.RenderStage.Static]);
...
function writeChunk(stageChunks, currentStage, chunk) {
...
var ValidationPrefetchKind = /*#__PURE__*/ function(ValidationPrefetchKind) {
...
async function createCombinedPayloadAtDepth(prefetchKind, initialRSCPayload, cache, initialLoaderTree, getDynamicParamFromSegment, query, depth, groupDepth, releaseSignal, boundaryState, clientReferenceManifest, stageEndTimes, useRuntimeStageForPartialSegments) {
    const workStore = _workasyncstorageexternal.workAsyncStorage.getStore();
    if (!workStore) {
        throw Object.defineProperty(new _invarianterror.InvariantError('createCombinedPayloadAtDepth must run inside a WorkStore'), "__NEXT_ERROR_CODE", {
            value: "E1184",
            enumerable: false,
            configurable: true
        });
   ...
   */ function wrapSlotsWithMarkers(slots, results) {
    ...
    async function buildNewTreeSeedData(lt, parentPath, key, segmentDepth) {
        ...
        const { mod: layoutOrPageMod, filePath: layoutOrPageFilePath } = await (0, _appdirmodule.getLayoutOrPageModule)(lt);
        ...
        if (layoutOrPageMod !== undefined) {
            ...
            if (instantConfig === null && validationLevel !== 'manual-warning' && validationLevel !== 'experimental-manual-error' && (0, _instantconfig.isImplicitValidationSegment)(segment) && !(0, _instantconfig.isFrameworkErrorRoute)(route)) {
                instantConfig = true;
            }
            ...
            case 1:
                {
                    if (useRuntimeStageForPartialSegments) {
                        ...
                        stage = _stagedrendering.RenderStage.ShellRuntime;
                    }
                    break;
                }
            case 3:
    ...
    switch(prefetchKind){
        case 1:
            {
                if (useRuntimeStageForPartialSegments) {
                    headStage = _stagedrendering.RenderStage.Runtime;
                } else {
                    headStage = _stagedrendering.RenderStage.ShellRuntime;
                }
                break;
            }
        case 3:
            {
                headStage = hasRuntimeSegments ? _stagedrendering.RenderStage.Runtime : _stagedrendering.RenderStage.Static;
                break;
            }
    }
    debug == null ? void 0 : debug(`    /_head - ${_stagedrendering.RenderStage[headStage]}`);
