...
async function runInstantInsightsWithTracing(ctx, fn) {
    ...
    return (0, _requestinsightsidentity.runWithRequestInsightsIdentity)({
        ...
        const span = (0, _localspanrecorder.createLocalSpan)({
        ...
        return (0, _localspanrecorder.withLocalSpan)(span, async ()=>{
            try {
                return await fn();
            } catch (err) {
 ...
 */ async function runValidationInDev(prefetchMode, instantInputs, staticInputs, ctx, fallbackRouteParams, devRenderDidError, validationAbortSignal) {
    ...
    if (needsInstantValidation && instantInputs) {
        ...
        const debugChunks = inputs.debugChannelClient ? await getDebugChunksOnce(inputs.debugChannelClient) : null;
        const hmrRefreshHash = (0, _workunitasyncstorageexternal.getHmrRefreshHash)(inputs.requestStore);
        const result = await validateInstantConfigs(prefetchMode, inputs.accumulatedChunks, debugChunks, inputs.startTime, rootParams, fallbackRouteParams, ctx, hmrRefreshHash, validationSamples, devRenderDidError, validationAbortSignal);
...
async function validateStaticShell(inputs, ctx, rootParams, fallbackRouteParams, debugChunks, hmrRefreshHash, validationAbortSignal) {
    ...
    const runtimeResult = await validateStagedShell(runtimeChunks, dynamicChunks, debugChunks, runtimeStageEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, hmrRefreshHash, _dynamicrendering.trackDynamicHoleInRuntimeShell, validationAbortSignal);
    ...
    const staticResult = await validateStagedShell(staticChunks, dynamicChunks, debugChunks, staticStageEndTime, rootParams, fallbackRouteParams, allowEmptyStaticShell, ctx, hmrRefreshHash, _dynamicrendering.trackDynamicHoleInStaticShell, validationAbortSignal);
 ...
 */ async function validateInstantConfigs(prefetchMode, accumulatedChunks, debugChunks, startTime, rootParams, fallbackRouteParams, ctx, hmrRefreshHash, validationSamples, devRenderDidError, validationAbortSignal) {
    const debug = process.env.NEXT_PRIVATE_DEBUG_VALIDATION === '1' ? console.log : undefined;
    ...
    }, debugChunks, startTime, clientReferenceManifest, createDebugChannel);
    const { implicitTags, nonce, workStore, isDebugChannelEnabled } = ctx;
    async function validateAtDepth(prefetchKind, depth, groupDepthForValidation) {
        return validateAtDepthImpl(prefetchKind, depth, groupDepthForValidation, null);
    }
    async function validateAtDepthImpl(prefetchKind, depth, groupDepthForValidation, previousBoundaryState) {
        ...
        const payloadResult = await createCombinedPayloadAtDepth(prefetchKind, initialRscPayload, cache, loaderTree, ctx.getDynamicParamFromSegment, ctx.query, depth, groupDepthForValidation, extraChunksSignal, boundaryState, clientReferenceManifest, stageEndTimes, useRuntimeStageForPartialSegments);
        ...
        if (previousBoundaryState === null && payloadResult.hasAmbiguousErrors) {
            ...
            const dynamicOnlyResult = await validateAtDepthImpl(prefetchKind, depth, groupDepthForValidation, boundaryState);
    ...
    for(let depth = maxDepth - 1; depth >= 0; depth--){
        ...
        for(let currentGroupDepth = maxGroupDepth; currentGroupDepth >= 0; currentGroupDepth--){
            ...
            const result = await validateAtDepth(prefetchKind, depth, currentGroupDepth);
...
async function validateInstantConfigInBuildWithSample(outerCtx, sample, allPossibleFallbackRouteParams, prefilledDataCache) {
    ...
    return _workasyncstorageexternal.workAsyncStorage.run(workStore, async ()=>{
        ...
        return await validateInstantConfigs(prefetchMode, accumulatedChunks, debugChunks, startTime, sampleRootParams, fallbackRouteParams, validationRenderCtx, undefined, validationSamples, false // build has no shared dev render that would surface errors
        );
    });
}
/**
